...
class ConferenceApi(remote.Service):
    ...
    def queryConferences(self, request):
        ...
        conferences = queries.get_conferences_with_filters(inequality_filter,
                                                           filters)
        names = queries.get_conferences_names(conferences)
        items = [utils.copyConferenceToForm(conf, names[conf.organizerUserId])
    ...
    def getConferencesCreated(self, user, request):
        ...
        items = [utils.copyConferenceToForm(conf, getattr(prof, 'displayName'))
                 for conf in confs]
    ...
    def getConferenceSessions(self, request):
        ...
        sessions = queries.get_conference_sessions(
            request.websafeConferenceKey)
        items = [utils.copySessionToForm(sess) for sess in sessions]
